...
/**
 * modificado del original de sf2.0.17
 * no esta terminado y no se esta usando
 */
 ...
 */
class DoctrineCrudGeneratorAdaptado extends DoctrineCrudGenerator
{
    private $filesystem;
    private $skeletonDir;
    private $routePrefix;
    ...
    public function generate(BundleInterface $bundle, $entity, ClassMetadataInfo $metadata, $format, $routePrefix, $needWriteActions)
    {
        ...
        $this->actions = $needWriteActions ? array('index', 'mostrar', 'crear', 'editar', 'eliminar') : array('index', 'mostrar');
        ...
        if (in_array('mostrar', $this->actions)) {
            $this->generateShowView($dir);
        }

        if (in_array('crear', $this->actions)) {
            $this->generateNewView($dir);
        }

        if (in_array('editar', $this->actions)) {
    ...
    {
        $this->renderFile($this->skeletonDir, 'views/mostrar.html.twig', $dir.'/mostrar.html.twig', array(
            'dir'               => $this->skeletonDir,
            'entity'            => $this->entity,
            'fields'            => $this->metadata->fieldMappings,
            'actions'           => $this->actions,
    ...
    {
        $this->renderFile($this->skeletonDir, 'views/crear.html.twig', $dir.'/crear.html.twig', array(
            'dir'               => $this->skeletonDir,
            'route_prefix'      => $this->routePrefix,
            'route_name_prefix' => $this->routeNamePrefix,
            'entity'            => $this->entity,
    ...
    {
        $this->renderFile($this->skeletonDir, 'views/editar.html.twig', $dir.'/editar.html.twig', array(
            'dir'               => $this->skeletonDir,
            'route_prefix'      => $this->routePrefix,
            'route_name_prefix' => $this->routeNamePrefix,
            'entity'            => $this->entity,
    ...
    private function getRecordActions()
    {
        return array_filter($this->actions, function($item) {
            return in_array($item, array('mostrar', 'editar'));
        });
    }
}
